...
class Rule {
    ...
    private void checkFinishedDependencies() {
        for (int successor : _depends.successors(_vertex)) {
            Rule r = _depends.getLabel(successor);
            if (r.isUnfinished()) {
                throw new
                IllegalStateException("error: dependencies not built");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        for (int successor : _depends.successors(getVertex())) {
            if (getTime() < _depends.getLabel(successor).getTime()) {
                return true;
            }
        }
        return false;
