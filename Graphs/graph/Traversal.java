...
public abstract class Traversal {
    ...
    public void traverse(Collection<Integer> V0) {
        _fringe.addAll(V0);
        HashSet<Integer> postVisited = new HashSet<Integer>();
            ...
            if (!marked(v)) {
                ...
                if (visit(v)) {
                    ...
                    if (!reverseSuccessors(v)) {
                        for (int edge : _G.successors(v)) {
                            if (processSuccessor(v, edge)) {
                                _fringe.add(edge);
                            }
                        }
                    } else {
                        for (int reverse = _G.outDegree(v) - 1;
                            reverse >= 0; reverse--) {
                            int r = _G.successor(v, reverse);
                            if (processSuccessor(v, r)) {
                                _fringe.add(r);
                            }
                        }
                    }
                ...
                if (shouldPostVisit(v) && !postVisited.contains(v)) {
                    postVisited.add(v);
                    if (!postVisit(v)) {
                        break;
                    }
    ...
    /** Marked sets of vertices. */
    protected final HashSet<Integer> _marked;
