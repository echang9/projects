...
class Game extends Observable {
        ...
        while (_exit < 0) {
            try {
                ...
                if (e.getMessage() == null) {
                    reportError("syntax error in command");
                } else {
    ...
    private void setAuto(Side color) {
        _playing = false;
        AI artifical = new AI(this, color);
        setPlayer(color, artifical);
        _inp.nextLine();
    }
    ...
    private Player getPlayer(Side color) {
        if (_players[color.ordinal()] == null) {
            if (color.equals(Side.RED)) {
    ...
    private void setSpots(int r, int c, int spots, String color) {
        ...
        } else if (spots > _board.neighbors(r, c)) {
            reportError("invalid request to put %d spots on square %d %d",
                spots, r, c);
        } else {
    ...
    private void setSize(int n) {
        _playing = false;
        if (n < 2 || n > 10) {
            reportError("size must be between 2 and 10", n);
        } else {
            _board.clear(n);
        }
        _inp.nextLine();
