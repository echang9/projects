...
abstract class Board extends Observable {
    ...
    boolean isLegal(Side player, int n) {
        if (!exists(n)) {
            return false;
        } else if (get(n).getSide().equals(Side.WHITE)
    ...
    boolean isLegal(Side player) {
        if (numOfSide(player) == size() * size()) {
            return false;
        } else if (!player.equals(whoseMove())) {
            return true;
        }
        return false;
    }
    ...
    final Side getWinner() {
        if (numOfSide(whoseMove()) == size() * size()) {
            return whoseMove();
        } else if (numOfSide(whoseMove().opposite()) == size() * size()) {
    ...
    public String toString() {
        ...
        for (int r = 1; r <= size; r++) {
            for (int c = 1; c <= size; c++) {
                ...
                s.append(get(r, c).getSpots() + side.substring(0, 1) + " ");
            }
        }
        s.append("\n" + "===");
        return s.toString();
